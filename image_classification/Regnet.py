...
class AnyNeSt(nn.Module):
    def __init__(self, ls_num_blocks, ls_block_width, ls_bottleneck_ratio, ls_group_width,
        ...
        self.linear=nn.Linear(ls_block_width[-1], 10)

    def forward(self, x):
        self.out1 = self.net(x)
...
class RegNet(AnyNeSt):
    def __init__(self, initial_width=32, slope=24.0, quantized_param=2.0, network_depth=16, bottleneck_ratio=1, group_width=32,
                 stride=2):
